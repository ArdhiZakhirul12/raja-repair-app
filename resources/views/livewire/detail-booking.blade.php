<div>

    {{-- @dd($bookings) --}}
    <div class="max-w-9xl mx-auto sm:px-6 lg:px-8">
        <div class="flex justify-between my-2 sm:my-2">
            <h1 class="text-2xl md:text-2xl text-gray-800 dark:text-gray-100 font-bold">
                Detail Transaksi Servis
            </h1>
            {{-- <button   class="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-700"
        onclick="document.getElementById('add-sparepart-modal').classList.remove('hidden')">
        Tambah Sparepart
    </button> --}}
        </div>
        <div class="flex">
            <div class="w-4/6 ">
                <div class="bg-white rounded shadow-md p-5 my-3">
                    <div class="flex items-center mb-2">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="30"
                            height="30" viewBox="0 0 30 30" fill="none" class="mr-2">

                            <image id="image0_76_221" width="30" height="30"
                                xlink:href="data:image/png;base64," />

                        </svg>
                        <h2 class="text-l font-semibold">Detail Data Customer</h2>
                    </div>
                    <hr>
                    <div class="p-4 mt-4 bg-white rounded shadow-sm">

                        {{-- <i class="fa-solid fa-user mr-3 text-blue-400"></i> --}}
                        <p class="text-l ml-2 mb-3 text-blue-500">Reihan Wudd Hibatullah</p>


                        <div class="flex mb-2">
                            <div>
                                <div class="flex mb-2 mr-4">
                                    <i class="fa-solid fa-phone mr-4 text-green-400"></i>

                                    <p class="text-gray-400">0891231231124</p>
                                </div>
                            </div>

                            <div>
                                <div class="flex mb-2">
                                    <i class="fa-solid fa-phone mr-4 text-green-400"></i>
                                    <p class="text-gray-400">0891231231124</p>
                                </div>
                            </div>


                        </div>

                        <div class="flex mb-2">
                            <i class="fa-solid fa-location-dot text-red-500 mr-4"></i>
                            <p class="text-gray-400">Jalan Cendrawasih No. 25, RT 03/RW 05, Kelurahan Mulyorejo,
                                Kecamatan
                                Sukomanunggal, Kota
                                Surabaya, Jawa Timur, 60112
                            </p>
                        </div>

                        {{-- <p>Pilih Teknisi</p> --}}


                        <div class="flex mb-2 ">
                            <i class="fa-solid fa-bars-progress text-orange-500 mr-3"></i>
                            <p class="text-gray-400">Kendala</p>
                        </div>


                        {{-- <div class="flex">
                        <div class="mt-4">
                            <label for="disabledInput" class="block text-sm font-medium text-gray-400">Disabled Input</label>
                            <input type="text" id="disabledInput" name="disabledInput" value="This is disabled" disabled class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-transparent rounded ">
                        </div>
    
                        <div class="mt-4">
                            <label for="disabledInput" class="block text-sm font-medium text-gray-400">Disabled Input</label>
                            <input type="text" id="disabledInput" name="disabledInput" value="This is disabled" disabled class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-transparent rounded ">
                        </div>
                    </div> --}}

                        <!-- Add more details as needed -->
                        {{-- <div class="flex">
                        <div class="mt-4">
                            <label for="disabledInput" class="block text-sm font-medium text-gray-400">Disabled Input</label>
                            <input type="text" id="disabledInput" name="disabledInput" value="This is disabled" disabled class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-transparent rounded ">
                        </div>
    
                        <div class="mt-4">
                            <label for="disabledInput" class="block text-sm font-medium text-gray-400">Disabled Input</label>
                            <input type="text" id="disabledInput" name="disabledInput" value="This is disabled" disabled class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-transparent rounded ">
                        </div>
                    </div> --}}

                        <!-- Add more details as needed -->
                    </div>
                </div>

                <div class="bg-white rounded shadow-md p-5 my-3">
                    <div class="flex items-center mb-2">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                            width="30" height="30" viewBox="0 0 30 30" fill="none">

                            <image id="image0_76_220" width="30" height="30"
                                xlink:href="data:image/png;base64," />

                        </svg>
                        <h2 class="text-l font-semibold">Detail Data HP</h2>
                    </div>
                    <hr>
                    <div class="p-4 mt-4 bg-white rounded shadow-sm">

                        {{-- <i class="fa-solid fa-user mr-3 text-blue-400"></i> --}}
                        <p class="text-l ml-2 mb-3 text-blue-500">MERK HP</p>


                        <div class="flex mb-2">
                            <div>
                                <div class="flex mb-2 mr-4">
                                    <i class="fa-solid fa-mobile mr-4 text-blue-400"></i>


                                    <p class="text-gray-400">MODEL</p>
                                </div>
                            </div>

                            <div>
                                <div class="flex mb-2">
                                    <i class="fa-solid fa-phone mr-4 text-green-400"></i>
                                    <p class="text-gray-400">IMEi</p>
                                </div>
                            </div>


                        </div>

                        <div class="flex mb-2">

                            <i class="fa-solid fa-gear text-red-500 mr-4"></i>
                            <div>
                                @for ($i = 0; $i < 3; $i++)
                                    <div class="p-2 bg-white rounded shadow-sm mb-2 flex items-center">
                                        <i class="fa-solid fa-circle-dot mr-2"></i>
                                        <p class="text-gray-400">Servis Layar Depan</p>
                                    </div>
                                @endfor
                            </div>
                         
                        </div>

                        {{-- <p>Pilih Teknisi</p> --}}


                        <div class="flex mb-2 ">
                            {{-- <i class="fa-solid fa-bars-progress "></i> --}}
                            <i class="fa-solid fa-screwdriver-wrench text-orange-500 mr-3"></i>
                            {{-- <p class="text-gray-400">Sparepart</p> --}}
                            <div class="flex flex-wrap">
                                @for ($i = 0; $i < 7; $i++)
                                    <div class="p-2 bg-white rounded shadow-sm mb-2">
                                        <p class="text-gray-400">Layar LCD 123</p>
                                    </div>
                                @endfor
                            </div>
                        </div>

                    </div>
                </div>
                @foreach ($bookings as $booking)
                    <p>ID: {{ $booking->id }}</p>
                    <p>Kode Pesanan: {{ $booking->kode_pesanan }}</p>
                    <p>Status: {{ $booking->status }}</p>
                @endforeach



            </div>
            <div class="w-2/6 p-4 m-3 bg-white rounded shadow-md h-100">
                <div class="flex items-center mb-2">
                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="30"
                        height="30" fill="none">

                        <image id="image0_26_74" width="30" height="30"
                            xlink:href="data:image/png;base64," />

                    </svg>
                    <h2 class="text-l font-semibold ">Teknisi</h2>
                </div>
                <label for="dataDropdown" class="block text-sm font-medium text-gray-400">List Data Teknisi</label>

                <select id="dataDropdown" name="dataDropdown"
                    class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"
                    onchange="showSuccessDialog()">
                    <option>Data 1</option>
                    <option>Data 2</option>
                    <option>Data 3</option>
                </select>

                <!-- Success Dialog -->
                <div id="successDialog"
                    class="hidden fixed top-20 right-10 bg-green-500 text-white p-4 rounded shadow-md transition-opacity duration-1000">
                    Success! Data teknisi has been updated.
                </div>

                <script>
                    function showSuccessDialog() {
                        const dialog = document.getElementById('successDialog');
                        dialog.classList.remove('hidden');
                        setTimeout(() => {
                            dialog.classList.add('hidden');
                        }, 2000); // Hide after 2 seconds
                    }
                </script>
            </div>
            <div class="overflow-hidden shadow-xl sm:rounded-lg bg-white dark:bg-gray-800 dark:text-slate-300">
            </div>
