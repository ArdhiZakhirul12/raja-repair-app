<x-app-layout>
    ...
    {{-- @dd($bookings->hpModel) --}}
    <div x-data="{ selectedStatus: 'diproses' }" class="p-4">
        <div class=" mx-6 my-3">
            <div class="bg-white rounded-lg shadow overflow-hidden l mx-auto p-4 ">
                
                <div class="bg-gray w-fit mb-4 rounded-lg">

                    <div class="flex flex-wrap gap-y-2 gap-x-4 px-2 py-2 bg-gray-100 dark:bg-gray-700/60 w-auto rounded-lg">
                
                        <button @click="selectedStatus = 'diproses'" 
                        :class="selectedStatus === 'diproses' ? 'bg-white ' : 'text-gray-400'"
                        class="px-4 py-2 rounded">Diproses</button>
                      
                        <button @click="selectedStatus = 'dikerjakan'" 
                ...
                @foreach ($claimGaransi as $index => $claim)
                    <div x-show="selectedStatus === '{{ $claim->status }}'">
                        <div class="border rounded bg-white flex items-center mb-4">
                            ...
                            <div class="p-4 text-center flex flex-col items-center">
                                ...
                                <h1 class="text-2xl font-bold">{{$claim->no_antrian}}</h1>
                            </div>
                            ...
                            <div class="w-2/3 p-4 flex items-center">
                                <div class="mr-6">
                                    <h1 class="text-2xl font-bold mb-3">{{ $claim->booking['kode_pesanan'] }}</h1>
                                    <p class="text-gray-400">{{ $claim->booking->hpModel->hpMerk->merk }}, {{ $claim->booking->hpModel->model }}</p>
                                </div>
                                <div class="flex flex-col items-start">
                                    <h1 class="text-l mb-3 text-gray-400">{{ $claim->booking->customer->nama }}</h1>
                                    <h1 class="text-l rounded p-2 text-center"
                                        :class="{
                                            'bg-blue-500 text-white': '{{ $claim['status'] }}' === 'diproses',
                                            'bg-yellow-500 text-white': '{{ $claim['status'] }}' === 'dikerjakan',
                                            'bg-green-500 text-white': '{{ $claim['status'] }}' === 'teknisi-selesai',
                                            'bg-gray-500 text-white': '{{ $claim['status'] }}' === 'selesai'
                                        }">
                                        {{ $claim['status'] }}
                                    </h1>
                            ...
                            <div class="w-1/3 p-4 flex justify-end">
                                <button class="bg-white hover:bg-blue-700 font-bold py-2 px-4 rounded shadow" onclick="window.location.href='{{ route('teknisi.claim.show', $claim->id) }}'">
                                    Detail
                                </button>
                            </div>
                        </div>
